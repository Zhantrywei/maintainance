    ...
    <div class="register">
        ...
        <el-form ref="registerform" :rules="rules" label-width="100px" size="medium" :inline="inline" label-position="right" :model="registerform">
            ...
            <el-form-item label="学生证照片" prop="StuCardImg" class="upload-img is-required" v-if="imgShow">
                <el-upload class="upload-demo" :data="{ 'stuId': this.registerform.stuId}" action="/api/user/upload" :before-upload="checkUpload" :on-preview="handlePreview" name="StuCardImg" :on-remove="handleRemove" :on-success="uploadSuccess" list-type="picture">
                    <el-button size="small" type="primary">点击上传</el-button>
                    <span slot="tip" class="el-upload__tip">只能上传jpg/png文件，且不超过500kb</span>
...
export default {
    ...
    methods: {
        ...
        submitForm: function(formName) {
            var that = this;
            if (this.registerform.usertype == "maintainer") {
        ...
        handleRemove(file, fileList) {
            console.log(file);
            try {
                if (file.response.result.filename == "IDCardImg") {
                    this.registerform.IDCardImg = null;
                    this.registerform.tmpIDCardImg = null;
                } else if (file.response.result.filename == "StuCardImg") {
                    this.registerform.StuCardImg = null;
                    this.registerform.tmpStuCardImg = null;
                }
            } catch(err) {
                // console.log(err)
            }
                    ...
                    .then(function(res) {
                        ...
                        if (res.data.status == 0) {
                            that.$alert(res.data.msg, "注册提示", {
                                ...
                                callback: action => {
                                    console.log("进入");
                                    var stuId = document.getElementById(
                                        "stuId"
                                    );
                                    stuId.focus();
                                }
        ...
        checkUpload() {
            ...
            // console.log("checkUpload",this.registerform.stuId);
            if (this.registerform.stuId == "") {
        ...
        uploadSuccess(response, file, fileList) {
            ...
            if (response.result.filename == "IDCardImg") {
                this.registerform.IDCardImg = response.result.imgUrl;
                this.registerform.tmpIDCardImg = response.result.tmpUrl;
            } else if (response.result.filename == "StuCardImg") {
                this.registerform.StuCardImg = response.result.imgUrl;
                this.registerform.tmpStuCardImg = response.result.tmpUrl;
            }
            if (this.registerform.IDCardImg != null) {
                this.showTipIDUpload = false;
            }
